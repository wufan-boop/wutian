...
async def _fetch_validate_data(data: dict) -> Dict[str, Any]:
    ...
    tasks = [
        call_sorftime_tool("product_detail", {"asin": asin, "keywordSupportSite": site}, mcp_url, api_key),
        call_sorftime_tool("competitor_product_keywords", {"asin": asin, "keywordSupportSite": site}, mcp_url, api_key),
        call_sorftime_tool("product_reviews", {"asin": asin, "keywordSupportSite": site}, mcp_url, api_key),
    ]
...
async def _fetch_compare_data(data: dict) -> Dict[str, Any]:
    ...
    for asin in asins[:3]:  # 最多3个
        tasks.append(call_sorftime_tool("product_detail", {"asin": asin, "keywordSupportSite": site}, mcp_url, api_key))
